...
export const listPaymentsController: RequestHandler = async (request, response, next) => {
  try {
    const data = await listPayments(String(request.params.saleNumber).toUpperCase());
...
export const createPaymentController: RequestHandler = async (request, response, next) => {
  try {
    ...
    const data = await createPayment(String(request.params.saleNumber).toUpperCase(), input, request.user!.sub);
