...
export function Sidebar({ modules, selected, onSelect, onLogout, user, compact }: SidebarProps) {
  const roleName = user.role?.name ?? user.roleId ?? 'Usuario';
      ...
      <ScrollView contentContainerStyle={styles.nav} horizontal={compact} showsHorizontalScrollIndicator={false}>
        {modules.map((module) => (
          <Pressable
            ...
            style={[styles.navItem, selected === module.label && styles.navItemActive]}
          >
            <View style={[styles.navGlyph, selected === module.label && styles.navGlyphActive]}>
              <Text style={[styles.navGlyphText, selected === module.label && styles.navGlyphTextActive]}>
                {module.shortLabel}
              </Text>
            </View>
            {!compact ? (
              <Text style={[styles.navText, selected === module.label && styles.navTextActive]}>
                {module.label}
              </Text>
            ) : null}
          </Pressable>
      ...
      {!compact ? (
        <View style={styles.sidebarFooter}>
          <View style={styles.userBadge}>
            <Text style={styles.userInitial}>{user.name.slice(0, 1).toUpperCase()}</Text>
          </View>
          ...
          <Pressable accessibilityLabel="Cerrar sesión" onPress={onLogout} style={styles.logout}>
